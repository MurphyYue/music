...
(function() {
	...
	createSongList(songIdList.id, function(data) {
		...
		$li.click(function() {
			...
			//			}else{
			//				$(oldPlayItems).each(function(i){
			//					if(oldPlayItems[i].id == _this.attr('data-id')) {
			//						return;
			//					} else {
			//						oldPlayItems.push(oldSong);
			//					}
			...
			if(songIsInArr($(this).attr('data-id'))) {
				oldPlayItems.push(oldSong);
			}
			
			
			function songIsInArr(obj) {
				for(let i=0;i<oldPlayItems.length; i++){
					if(oldPlayItems[i].id==obj){return false}
					else{return true}
				}
			}
		})
